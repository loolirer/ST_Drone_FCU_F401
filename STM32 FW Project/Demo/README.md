...
A stripped build of `Official release - 170120`. On power-up it flies a fixed, open-loop
throttle profile while the original cascade PID holds the attitude level. No radio,
no BLE and no USB.

The source release itself is untouched. This is a standalone copy.
...
The board must be flat and completely still for the first 4 seconds. The gyro and accel
offsets and the level reference are captured in that window, and nothing verifies it. If the
board moves during calibration the level reference is wrong and the drone will fly off at an
angle.

This holds attitude, not altitude. There is no altitude controller anywhere in this codebase.
No rangefinder, no optical flow, and the barometer is not used for control. `PWM_HOVER` is an
open-loop guess, so whether the drone hovers, climbs or sinks depends on battery voltage,
payload and prop condition. It will also drift horizontally, since nothing measures position.

Check the motor numbering and prop direction before the first spin-up. The mixer assumes
motors 1 and 3 spin one way and 2 and 4 the other, in the X layout. Wrong wiring flips the
airframe instantly and no amount of PID will save it.

## The one number you have to calibrate

`PWM_HOVER` in `Src/main.c`. It is currently 1100, which is a guess.
...
3. Mind the headroom. `set_motor_pwm()` clamps at 1900 and the PID can add up to 800 counts
   per axis in either direction, so above roughly 1100 the mixer starts saturating during
   corrections and the attitude loop loses authority.
...
| `TILT_ABORT_RAD` | 0.87 | about 50°, cuts motors and latches off |

`HARD_CEILING_TICKS` is a second, independent limit. Nothing runs longer than the profile
plus 3 s, whatever else happens.
...
`gTHR` survives as a synthetic arm flag rather than a throttle. `ahrs.c` and
`flight_control.c` both key their `< MIN_THR` behaviour off it, so it still switches `AHRS_KP`
between 10.0 and 0.4 and still gates the integrator resets.

## Removed from the build

- Sources: `rc.c`, `sensor_service.c`, `console.c`, `timer.c`, `usb_device.c`, `usbd_*.c` and
  their headers. Also `PID.c`, `filter.c`, `stm32f4xx_nucleo.c`, `main_backup.c` and
  `main_davide_debug.c`, which were never in the CubeIDE project to begin with.
- Middlewares: the whole folder. BlueNRG and the USB Device Library are both gone.
- Drivers: `BSP/Components/spbtlerf` (the BlueNRG driver) and `BSP/Board`. `board.c` includes
